         require("dotenv").config(), 
          Response = require('../util/response_manager'),
          HttpStatus = require('../util/http_status'),
          express = require('express'),
          nodemailer = require('nodemailer'),Customer = require("../models/customer");
           const router = express.Router();
          const { MAIL_USERNAME, MAIL_PASSWORD, MAIL_SERVICE } = process.env;router.use(require("body-parser").urlencoded({extended: true}));
         module.exports = {
         sendMail(req, res){const transporter = nodemailer.createTransport({service: MAIL_SERVICE,
                 auth: {
                user: MAIL_USERNAME,
                pass: MAIL_PASSWOR
            }
        ...
        Customer.findById(id, (err, foundCustomer)=>{
            if(err){
                ...
                if(foundCustomer.email  && foundCustomer.email != "Not set" || undefined){
                    ...
                    subject = req.body.subject; text = req.body.text;const params = {
                        from: MAIL_USERNAME,
                        to: recipient,
                        subject: subject,
                        text: text };
